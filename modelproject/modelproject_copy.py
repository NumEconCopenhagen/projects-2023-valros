...
class SolowModelClass:
    ...
    def simulate(self, periods = 100, ext=False, do_print=False):
        ...
        # d. evaluate if imaginary part is irrelevant
        ...
        if do_print == True:
            ...
            ax[0,1].axhline(y=ss, color='black', label=f'Steady state {ss:.4f}')
            ax[0,1].legend()
            ax[0,1].set_title('Capital-output ratio, $z_t$')
            ax[1,1].plot(sim.t,sim.A, label=r'$A_t$')
            ax[1,1].plot(sim.t,sim.L, label=r'$L_t$')
            ax[1,1].legend()
            ax[1,1].set_title('Technology and technology, $A_t$ and $L_t$')
